...
class SJRenderer {
  ...
  renderDOM(elem, scope) {
    ...
    const modelValue = modelName? sjExpression.evaluateExpression(scope, modelName, this.targetElement) : null;
  ...
  shouldHideElement(elem, scope) {
    ...
    if (cond) {
      const val = sjExpression.evaluateExpression(scope, cond, this.targetElement);
      if (!val) {
        return true;
      }
    }
    ...
    if (attrName.startsWith('sj-')) {
      ...
      if (event) {
        IncrementalDOM.attr(event, (e) => {
          ...
          sjExpression.evaluateExpression(currentScope, attr.value, this.targetElement);
        });
      } else if (attr.name === 'sj-model') {
        isModelAttribute = attr.value;
        IncrementalDOM.attr("onchange", (e) => {
          sjExpression.setValue(scope, attr.value, e.target.value);
          this.render();
        });
        if (!scope[attr.value]) {
        ...
        const result = sjExpression.evaluateExpression(scope, attr.value);
        if (result) {
  ...
  replaceVariables(label, scope) {
    return label.replace(/\{\{([$A-Za-z0-9_.-]+)\}\}/g, (m, s) => {
      if (s === '$_') {
        ...
        return sjExpression.evaluateExpression(scope, s, this.targetElement);
      }
    });
  }
